...
async function processWebhookQueueWorker() {
    try {
        const pendingItems = await db.getPendingWebhooks(10);
        for (const item of pendingItems) {
            try {
                ...
                await db.updateWebhookAttempt(item.id, 'SUCCESS', null, Date.now());
                await db.updateOrderWebhookStatus(item.qrisId, 'SUCCESS');
                ...
                if (nextAttempts >= item.maxAttempts) {
                    await db.updateWebhookAttempt(item.id, 'FAILED', errorMsg, Date.now());
                    await db.updateOrderWebhookStatus(item.qrisId, 'FAILED');
                    ...
                    await db.updateWebhookAttempt(item.id, 'PENDING', errorMsg, nextAttemptAt);
